...
interface SafeCategory
  extends Pick<Category, 'id' | 'name' | 'hasChildren' | 'MetaTagDescription' | 'Title'> {
  url: string | null
...
export const resolvers = {
  Category: {
    ...
    href: async ({ id, url }: SafeCategory, _: any, { clients: { catalog } }: Context) => {
      if (url == null) {
    ...
    slug: async ({ id, url }: SafeCategory, _: any, { clients: { catalog } }: Context) => {
      if (url == null) {
        const category = await getCategoryInfo(catalog, id, 4)
        url = category.url
      }
    ...
    children: async ({ id, children }: SafeCategory, _: any, { clients: { catalog } }: Context) => {
      if (children == null) {
        const category = await getCategoryInfo(catalog, id, 4)
